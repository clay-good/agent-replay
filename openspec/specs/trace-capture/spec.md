...
#### Scenario: A session opening with an injected skill preamble

- **WHEN** a transcript's first user turns are marked `isMeta` and a later turn is a person's question
- **THEN** the question becomes `input.prompt` and the injected turns are kept in `metadata.preamble_prompts`
